         ...
         <form method = "POST" >
          <div class="form-row">
            <div class="form-group col-md-6">
              ...
              <input type="text" class="form-control" required id="permission_title" name = "permission_title" value = "<?= $group_info['name'] ?>" placeholder="Digite o nome do grupo...">
            </div>
          </div>
          <div class = "form-row">
            <div class="form-group col-md-6">
              ...
              <select multiple class="form-control" id="list_permissions" name = "list_permissions[]">
                <?php 
                  foreach($permissions_list as $p) {
                    ?>
                      <option <?= (in_array($p['id'], $group_info['params'])) ? "selected=selected" : "" ?> value = "<?= $p['id'] ?>" ><?= $p['name'] ?></option>
                    <?php 
                  } 
                ?>
              </select>
